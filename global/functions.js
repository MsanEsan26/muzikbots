...
module.exports = (bot, utils, ytdl, config) => {
    ...
    fs.readdir("./commands/", (err, files) => {
        ...
        if (jsfiles.length <= 0) return console.log("Başlamak İçin Komut Yok...");

        console.log(`${jsfiles.length} Komut Var...`);
        jsfiles.forEach((f, i) => {
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            if (playlist) return;
            else return message.channel.send(`🎵 **${music.title}** Kuyruğa Eklendi`);
        }
        return;
    }
    ...
    bot.play = (guild, music) => {
        ...
        queue.textChannel.send(`🎵 **${music.title}** Şimdi Çalıyor!`);
    }
